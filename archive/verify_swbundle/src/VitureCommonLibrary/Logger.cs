...
public static class Logger
{
	...
	public static void Initialize(string config_file = "Assets/Configs/Nlog.config")
	{
		...
		LogManager.Configuration = (LoggingConfiguration)new XmlLoggingConfiguration(config_file);
	...
	public static void InitializeConsoleLogger()
	{
		...
		try
		{
			...
			ConsoleTarget val2 = new ConsoleTarget("console")
			{
				Layout = Layout.op_Implicit("${longdate} ${uppercase:${level}} - ${message}")
			};
			val.AddRule(LogLevel.Trace, LogLevel.Fatal, (Target)(object)val2, "*");
