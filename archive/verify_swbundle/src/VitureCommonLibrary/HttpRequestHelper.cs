...
public static class HttpRequestHelper
{
	...
	public static async Task<string?> RequestJsonContent(string url)
	{
		...
		catch (JsonException val)
		{
			...
			Logger.Error("RequestJsonContent: JSON parse error, url=" + url + ", msg=" + ((Exception)(object)val2).Message, ((Exception)(object)val2).StackTrace);
		}
		catch (Exception ex3)
		{
			Logger.Error($"RequestJsonContent: unexpected error [{ex3.GetType().Name}] after {sw.ElapsedMilliseconds} ms, url={url}, msg={ex3.Message}", ex3.StackTrace);
		}
		return string.Empty;
	}
